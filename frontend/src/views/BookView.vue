...
const fetchBookDetails = async () => {
  try {
    ...
    if (!result || !result.volumeInfo) {
      throw new Error('Livro não encontrado ou resposta inválida da API.');
    }
    book.value = result;
  } catch (error) {
    ...
    book.value = null;
    errorMessage.value =
      'Não foi possível carregar os detalhes do livro. Verifique sua conexão ou tente novamente mais tarde.';
  }
};
...
<template>
  ...
  <div v-if="book">
    ...
    <div class="container mx-auto p-4">
      ...
      <Rating v-if="avaliacoes && Array.isArray(avaliacoes) && avaliacoes.length > 0" modo="book" />
    </div>
  </div>
